...
async function generateAudio(poiId, lang) {
  return await apiCall("POST", "/api/audio/admin/generate", {
    poiId,
    languageCode: lang,
  });
}
...
async function deleteVendor(id) {
  ...
  if (!res.ok)
    throw new Error(
      data.message || `Ngưng hợp tác thất bại (HTTP ${res.status})`,
    );
  return data;
}
...
async function approveVendor(vendorId, poiPointId = 0) {
  return await apiCall("PUT", `/api/admin/vendors/${vendorId}/approve`, {
    poiPointId,
  });
}
...
async function rejectVendor(vendorId, reason = "") {
  return await apiCall("PUT", `/api/admin/vendors/${vendorId}/reject`, {
    reason,
  });
}
...
async function rejectPendingUpdate(id, reason = "") {
  return await apiCall("POST", `/api/admin/pending-updates/${id}/reject`, {
    reason,
  });
}
...
async function approveStagingImage(id, poiPointId = 0) {
  return await apiCall("POST", `/api/admin/staging-images/${id}/approve`, {
...
async function rejectStagingImage(id, reason = "") {
  return await apiCall("POST", `/api/admin/staging-images/${id}/reject`, {
    reason,
  });
}
...
async function approveLogo(id, poiPointId = 0) {
  return await apiCall("POST", `/api/admin/staging-images/logo/${id}/approve`, {
    PoiPointId: poiPointId,
  });
}
...
async function rejectLogo(id, reason = "") {
  return await apiCall("POST", `/api/admin/staging-images/logo/${id}/reject`, {
    reason,
  });
}
...
async function approveLogoDeletion(id) {
  return await apiCall(
    "POST",
    `/api/admin/staging-images/logo/${id}/approve-deletion`,
  );
}
...
async function rejectLogoDeletion(id, reason = "") {
  return await apiCall(
    "POST",
    `/api/admin/staging-images/logo/${id}/reject-deletion`,
    { reason },
  );
}
...
async function approveDeletionRequest(id, adminNote = "") {
  return await apiCall(
    "POST",
    `/api/admin/staging-images/${id}/approve-deletion`,
    { adminNote },
  );
}
...
async function rejectDeletionRequest(id, reason = "") {
  return await apiCall(
    "POST",
    `/api/admin/staging-images/${id}/reject-deletion`,
    { reason },
  );
}
